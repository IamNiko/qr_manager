...
from servicios.mensajes import dividir_mensaje, es_agradecimiento
from base_datos.modelos import Base
from base_datos.sesion import engine, obtener_db
from servicios.prompts import gestor_prompts
from nucleo.validadores import es_nombre_valido, limpiar_nombre, detectar_rol  # Cambiado a los nombres en español
...
def manejar_interaccion(remitente: str, mensaje_entrante: str, db) -> dict:
    """
    Maneja la interacción del chatbot basada en el estado del usuario.

    Args:
        remitente (str): Identificador único del usuario.
        mensaje_entrante (str): Mensaje enviado por el usuario.
        db: Sesión activa de la base de datos.

    Returns:
        dict: Respuesta del bot y posibles opciones.
    """
    logging.debug(f"Sesión de base de datos: {db}")
    if db is None:
        raise ValueError("Se requiere una sesión de base de datos activa para manejar la interacción.")

    estado_usuario = gestor_estados.obtener_estado(remitente, db=db)
    ...
    if estado_usuario["paso"] == "inicio":
        ...
        gestor_estados.actualizar_estado(remitente, estado_usuario, db)

    # FLUJO DE NOMBRE
    elif estado_usuario["paso"] == "nombre":
        if not estado_usuario.get("nombre"):  # Si aún no tiene nombre
    ...
    # DETALLES DE PROBLEMAS (JUGADOR/STAFF)
        ...
        if estado_usuario["paso"] == "detalle_otros":
            if mensaje_entrante == "1":  # Información sobre funcionamiento
            ...
            elif mensaje_entrante == "2":  # Políticas de reembolso
                content = (
                ...
                if "efectivo" in mensaje_entrante.lower() or "metalico" in mensaje_entrante.lower():
                    content += " El usuario pregunta sobre pago en efectivo. Explica amablemente que solo se aceptan pagos electrónicos."

            try:
                if not respuesta_bot:  # Si no se estableció en la opción 1
                    respuesta_bot = obtener_respuesta_gpt4(content, estado_usuario, db)
...
# Agregar nuevo elif para manejar consultas de funcionamiento
        ...
        if es_agradecimiento(mensaje_entrante):
            respuesta_bot = (
                ...
                "no dudes en preguntarme. Estoy aquí para ayudarte."
            )
        elif "efectivo" in mensaje_entrante.lower() or "metalico" in mensaje_entrante.lower():
            respuesta_bot = (
                f"Lo siento {estado_usuario['nombre']}, pero la máquina no acepta pagos en efectivo. "
                "Solo se aceptan pagos con:\n"
                "✓ Tarjeta bancaria\n"
                "✓ Contactless\n"
                "✓ Código QR\n"
                "Para cualquier otra consulta, estoy aquí para ayudarte."
            )
        else:
            try:
    ...
    return {"respuesta": partes_mensaje}
